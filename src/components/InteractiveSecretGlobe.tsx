...
export default function InteractiveSecretGlobe() {
  ...
  const handleMove = (clientX: number, clientY: number) => {
    ...
    setRotationY(Math.max(-45, Math.min(45, rotationStartRef.current.y + deltaY * speed)));
  };
  ...
  return (
    <div className="mt-16 bg-gradient-to-b from-[#060714] via-[#030409] to-[#010104] border border-indigo-500/10 rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden text-right" dir="rtl">
      ...
      <div className="mb-14 p-5 rounded-2xl bg-gradient-to-r from-red-950/40 via-[#0d0712] to-red-950/40 border border-red-500/20 shadow-[0_0_20px_rgba(239,68,68,0.05)] relative overflow-hidden">
        ...
        <p className="text-gray-300 text-xs mt-3 leading-relaxed border-t border-white/5 pt-3 font-normal">
          <span className="text-red-400 font-bold ml-1">[تحذير سري للغاية]:</span>
          هذه الملاحظات والمعلومات محمية ومصنّفة للرواد الملتزمين أصحاب الهمم العالية والركائز الفائقة فقط. يمنع تداولها أو مشاركتها خارج حدود الملاحة لتبقى سراً مصوناً لوقود إنتاجيتكم في الأسطول. تصفحك وتديرك للكرة هو فك التشفير الشخصي الآمن لك.
        </p>
      </div>
      ...
      <div className="flex flex-col items-center justify-center relative min-h-[580px] py-6">
              ...
              <div className="bg-[#040513]/85 backdrop-blur-3xl border border-indigo-500/20 rounded-[2.5rem] p-6 md:p-8 text-center relative shadow-[0_25px_60px_rgba(0,0,0,0.8),0_0_35px_rgba(99,102,241,0.12)] overflow-hidden">
                ...
                <div className="flex items-center justify-center gap-4 mt-5.5 pt-3 border-t border-white/5 font-mono text-[9px] text-gray-500">
                  <div>
                    ...
                    <span className="text-emerald-400 font-bold">{currentSecret.impactScore}</span>
                  </div>
                  <div className="w-1.5 h-1.5 rounded-full bg-white/10" />
                  <div>
                    <span>الحالة: </span>
                    <span className="text-cyan-400 font-bold">مشفّر وآمن للأسطول</span>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-60 md:mt-56 z-10">
          ...
          >
            ...
            <div className="w-full h-full rounded-full overflow-hidden bg-gradient-to-tr from-[#010207] via-[#04081c] to-[#040d3a] relative shadow-[inset_0_4px_25px_rgba(0,0,0,0.95),_inset_0_-10px_35px_rgba(99,102,241,0.18)] flex items-center justify-center">
              ...
              >
                <g fill="none" stroke="rgba(99, 102, 241, 0.22)" strokeWidth="1">
                  {/* Grid Lines simulating 3D Latitude/Longitude mesh */}
                  <path d="M 0,30 L 900,30 M 0,60 L 900,60 M 0,90 L 900,90 M 0,120 L 900,120 M 0,150 L 900,150 M 0,180 L 900,180 M 0,210 L 900,210 M 0,240 L 900,240 M 0,270 L 900,270" strokeDasharray="3,3" />
                ...
                <g fill="rgba(99, 102, 241, 0.12)" stroke="rgba(99, 102, 241, 0.28)" strokeWidth="1.2">
                  {/* Continent Block 1 */}
            ...
            {!hasInteracted && (
              <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-[#04081c] border border-indigo-500/30 text-[8px] font-mono text-indigo-300 rounded-full px-3 py-1 animate-bounce select-none pointer-events-none tracking-widest whitespace-nowrap shadow-md">
                <RefreshCw className="w-2.5 h-2.5 animate-spin" /> اسحب الأرض بكل الاتجاهات للملاحة
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-3.5 text-center z-10 select-none">
          <p className="text-[10px] text-gray-500 font-mono tracking-widest uppercase">
            مستند سري مفكوك: {activeSecret + 1} من {CLASSIFIED_SECRETS.length} • الاتصال المداري آمن آلياً 🔐
          </p>
        </div>
